...
draw_gganno <- function(anno, order_list, heat_matrix, id, draw = TRUE) {
    which <- anno@which
...
#' @inheritDotParams grid::viewport -width -height
#' @method draw ggAnnotationFunction
#' @export
methods::setMethod(
    f = "draw",
    ...
    definition = function(object, index, ...) {
        if (ht_opt$verbose) {
            cli::cli_inform(
                "draw annotation generated by {.fn object@fun_name}"
            )
        }
        ...
        p <- draw_gganno(object, order_list, id = "gganno", draw = FALSE)
        ...
        cur_vp <- grid::current.viewport()
        vp_name1 <- cur_vp$name
        print(p, vp = cur_vp)
        vp_name2 <- grid::current.viewport()$name
        if (vp_name1 != vp_name2) {
            cli::cli_abort(
                "Viewports should be the same before and after plotting the annotation graphics."
            )
        }
        grid::popViewport()
    }
