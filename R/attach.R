# copy from https://github.com/tidyverse/tidyverse/blob/main/R/attach.R
unloaded_pkgs <- function(core = c("ggplot2", "ComplexHeatmap")) {
...
eheat_attach <- function() {
    to_load <- unloaded_pkgs()
    suppressPackageStartupMessages(lapply(to_load, same_library))
    invisible(to_load)
}

eheat_attach_message <- function(to_load) {
    if (length(to_load) == 0) {
        return(NULL)
    }

    header <- cli::rule(
        left = cli::style_bold("Attaching core eheat packages"),
        right = paste0("eheat ", package_version_h("eheat"))
    )
